...
- `POST /auth` público, integrado à **Lambda de autenticação por CPF**
  ([US-F3-01](../../../docs/user-stories/f3-01-serverless-cpf-auth.md));
...
- **throttling**, **CORS** e **access logs JSON** no CloudWatch (insumo da
  [US-F3-10](../../../docs/user-stories/f3-10-observabilidade-apm.md)).

> **Migração (US-F3-07):** este stage vive aqui temporariamente; na segregação
> em 4 repositórios ele migra para `oficina-auth-lambda` ou `oficina-infra-k8s`.
>
> **Por que AWS API Gateway (e não Kong/Traefik in-cluster)?** Decisão
> registrada em ADR ([f3-doc-02](../../../docs/user-stories/f3-doc-02-adrs.md)).
